...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.happened) {
    ...
    // Apply penalty if borrower's fault
    if (blame === "borrower") {
      ...
      await notify(swap.request.borrower.id, "PENALTY", `Your borrow limit was reduced due to a missed swap for "${swap.request.book.title}"`, `/swaps/${id}`);
    }
  }
