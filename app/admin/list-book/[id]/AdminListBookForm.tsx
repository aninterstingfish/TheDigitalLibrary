...
export default function AdminListBookForm({ childId, childName }: { childId: string; childName: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/admin/list-book/${childId}`, {
      method: "POST",
  ...
  if (done) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
        ...
        <p className="font-semibold text-black mb-1">Book listed!</p>
        <p className="text-gray-400 text-sm mb-6">&ldquo;{title}&rdquo; has been added to {childName}&apos;s library.</p>
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setDone(false); setTitle(""); setAuthor(""); setSeries(""); setSeriesNumber(""); setLabelType("PERSONAL"); setTags([]); setTagInput(""); setCondition(""); setGenres([]); setDescription(""); setCoverPreview(null); setCoverFile(null); }}
            className="px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-black hover:bg-gray-50 transition-all">
            List another
          </button>
    ...
    <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-2xl border border-gray-100 p-6">
      <div>
        ...
        <div className="flex items-start gap-4">
          <button type="button" onClick={() => fileRef.current?.click()} className="group relative" aria-label="Upload cover photo">
            {coverPreview ? (
      ...
      <div>
        ...
        <input id="title" type="text" value={title}
          ...
          placeholder="e.g. To Kill a Mockingbird" className={ic(!!errors.title)} />
      ...
      <div>
        <label className="block text-sm font-medium text-black mb-2">Genres <span className="text-gray-400 font-normal">(optional)</span></label>
      ...
      <div>
        <label htmlFor="description" className="block text-sm font-medium text-black mb-1.5">Notes <span className="text-gray-400 font-normal">(optional)</span></label>
